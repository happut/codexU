...
- 菜单栏任务状态灯：在额度内容右侧通过竖线隔开的横向红黄绿灯控展示本机Codex hook状态；`PreToolUse`点亮黄灯，`PermissionRequest`点亮红灯，任务结束点亮绿灯（当前Codex对应`Stop`事件，并兼容`TaskCompleted`）。在设置中启用后，需重启Codex并使用`/hooks`审核、信任配置。hook使用稳定的本机helper路径，瞬时状态写入Codex沙箱可访问的用户临时目录，不依赖应用安装位置；状态协议已按Runtime区分，为Claude Code hook接入预留扩展点。
