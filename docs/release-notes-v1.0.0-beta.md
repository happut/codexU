...
这是 codexU 的 beta03 版本，重点补齐已安装用户的 GitHub Release 更新检测，并继续收敛设置窗口的交互密度。已安装 beta02 的用户可以在设置的系统区手动检查更新，也可以等待默认的每日自动检查提示。本版同时调整关闭主窗口后的后台行为：保留菜单栏状态项，隐藏Dock图标。
