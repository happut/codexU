...
- 简约模式以缩小后的 Runtime 图标为中心，5h 和 7d 使用外围同心双环；Logo 必须与双环共用圆心并完整落在内环安全区，不得与环线重叠或粘连。经典模式的独立微型环内只保留百分比数字；丰富模式使用带标签的线性进度条。
...
- 经典和丰富模式的 Runtime 图标使用 18pt 常规状态栏尺寸；简约模式使用 9pt Logo，并以外接矩形角点为边界验证与内环至少 0.75pt 的径向间隙。
