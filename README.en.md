...
- Menu bar agent light: uses a horizontal red-yellow-green control on the right, separated from quota content by a vertical divider, for local Codex hook state. `PreToolUse` activates yellow, `PermissionRequest` activates red, and task completion activates green (the current Codex event is `Stop`, with `TaskCompleted` also recognized). Enable it in Settings, restart Codex, then review and trust the configuration with `/hooks`. Hooks use a stable local helper path and keep transient state in the user temporary directory that the Codex sandbox can access. The status protocol is runtime-scoped for a future Claude Code adapter.
